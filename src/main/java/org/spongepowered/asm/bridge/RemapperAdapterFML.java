...
public final class RemapperAdapterFML extends RemapperAdapter {
    ...
    private RemapperAdapterFML(Remapper remapper, Method methodUnmap) {
        super(remapper);
        logger.info("Initialised Mixin FML Remapper Adapter with {}", remapper);
        mdUnmap = methodUnmap;
    }

    public String unmap(String typeName) {
        try {
            return mdUnmap.invoke(remapper, typeName).toString();
